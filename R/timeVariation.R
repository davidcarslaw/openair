...
#'   `RColorBrewer` colours --- see the [openColours()] function for more
#'   details. For user defined the user can supply a list of colour names
...
#' @param panels The plots to use in the lower row of the [timeVariation()]
#'   plot, defaulting to `c("hour", "month", "day")`. Users may wish to instead
#'   provide `c("hour", "day", "month")` for a more logical order. Only
#'   specified panels are drawn. For example, if only one month's data is
#'   provided, users could provide `c("hour", "day")` to only draw the "hour"
#'   and "weekday" lower panels and exclude the "month" panel.
...
#' pollutant = "pm10",
#' panels = c("hour", "day")) # exclude superfluous 'month' panel
#' }
#'
...
timeVariation <- function(mydata,
                          ...
                          auto.text = TRUE,
                          alpha = 0.4,
                          panels = c("hour", "month", "day"),
                          plot = TRUE,
                          month.last,
                          ...) {
  if (!missing(month.last)) {
    cli::cli_warn(
      "{.arg month.last} has been deprecated. Please use the {.arg panels} argument to reorder the lower panels of {.fun timeVariation}."
    )
  }
  ...
  main.plot <- function(...) {
    # check panels are correct
    panels <- unique(panels)
    rlang::arg_match(panels, c("hour", "month", "day"), multiple = TRUE)
    ...
    # create list of possible panels
    theplots <- list("hour" = hour, "month" = month, "day" = day)
    # filter by user-specified panels
    theplots <- theplots[panels]
    # get horizontal bounds for the number of panels
    bounds <- seq(0, 1, length.out = length(panels) + 1)
    # iteratively plot lower panels
    for (i in seq_along(theplots)) {
      print(
        theplots[[i]],
        position = c(bounds[i], y.dwn, bounds[i + 1], 0.53),
        more = i != max(seq_along(theplots))
      )
    }
