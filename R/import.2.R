import.2 <- function(file = file.choose(), file.type = "csv", header.at = 1,
  ...
  # file names read in
  #################
  file.names <- if (is.null(header.at) || header.at < 1) {
  ...
  if (is.null(header.at) || is.numeric(header.at) && header.at < 1) {
    temp <- unlist(lapply(
      c("date.name", "time.name", "is.ws", "is.wd", "is.site"),
      function(x) if (!is.null(get(x)) & !is.numeric(get(x))) x
    ))
    if (!is.null(temp)) {
      stop(
        "Invalid import options for data import without header,\n",
  ...
  if (!is.null(temp)) {
    stop(
      "Invalid import option combination,\n",
      "       [conflicting options: ",
      paste(temp, collapse = ", "), "]",
      call. = FALSE
    )
  ...
  if (bad.24 == TRUE) {
    ...
    a[grep(bad.time, b, ignore.case = TRUE)] <-
      as.POSIXct(
        as.POSIXlt(strptime(
          as.character(b[grep(bad.time, b, ignore.case = TRUE)]),
