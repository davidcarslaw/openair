...
##' user can explicitly specify the interval through \code{interval} in the same
##' format as \code{avg.time} e.g. \code{interval = "month"}. It may also be
##' useful to set \code{start.date} and \code{end.date} if the time series do
##' not span the entire period of interest. For example, if a time series ended
##' in October and annual means are required, setting \code{end.date} to the end
##' of the year will ensure that the whole period is covered and that
##' \code{data.thresh} is correctly calculated. The same also goes for a time
##' series that starts later in the year where \code{start.date} should be set
...
##'   successive intervals so that \code{timeAverage} can work out how much
##'   \sQuote{padding} to apply. To pad-out data in this way choose \code{fill =
##'   TRUE}.
...
##'   \code{start.date} and \code{end.date} to see whether it is advisable to
##'   set these other options.
##'
##' @param statistic The statistic to apply when aggregating the data; default
##'   is the mean. Can be one of \dQuote{mean}, \dQuote{max}, \dQuote{min},
##'   \dQuote{median}, \dQuote{frequency}, \dQuote{sum}, \dQuote{sd},
##'   \dQuote{percentile}. Note that \dQuote{sd} is the standard deviation,
##'   \dQuote{frequency} is the number (frequency) of valid records in the
##'   period and \dQuote{data.cap} is the percentage data capture.
##'   \dQuote{percentile} is the percentile level (\%) between 0-100, which can
##'   be set using the \dQuote{percentile} option --- see below. Not used if
...
##'   calculations where the \code{data.thresh} >0. For the vast majority of
...
##'   interval explicitly e.g. \code{interval = "month"}. Users can also supply
##'   a time interval to \emph{force} on the time series. See \code{avg.time}
##'   for the format.
##'
##'   This option can sometimes be useful with \code{start.date} and
##'   \code{end.date} to ensure full periods are considered e.g. a full year
##'   when \code{avg.time = "year"}.
...
##'   than the original time series, data are \sQuote{padded out} with
##'   \code{NA}. To \sQuote{pad-out} the additional data with the first row in
##'   each original time interval, choose \code{fill = TRUE}.
