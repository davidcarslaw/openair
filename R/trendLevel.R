...
#' and diurnal variations. However, x, y and type and summarising statistics
#' can all be modified to provide a range of other similar plots.
#'
...
#' of named parameters and sent using \code{stat.args}. For example, the
...
#' function should return the required answer as a numeric or \code{NA}. Note:
#' If the supplied function returns more than one answer, currently only the
#' first of these is retained and used by \code{trendLevel}. All other returned
...
#' may fail. Note: The \code{stat.safe.mode = TRUE} option returns an NA
...
#'   data into four levels. (Notes: This option is ignored for date
#'   conditioning and factors.  If less than three values are supplied, three
#'   values are determined by recursion; if more than three values are
...
#'   this case. \code{labels} must also be supplied.
...
#'   with such functions. For a very few cases, e.g. for a function that
#'   counted missing entries, it might need to be set to \code{FALSE} (see
#'   Details below.)
...
#'   e.g. a hour of the day when no measurements were taken. Empty \code{x} and
